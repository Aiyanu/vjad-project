...
export const getReferrals = async (req: AuthenticatedRequest, res: Response) => {
  try {
...
export const getReferralCount = async (req: AuthenticatedRequest, res: Response) => {
    try {
        if (!req.user) return apiError(res, "Unauthorized", 401);
        ...
        return apiSuccess(res, { count: affiliate?._count.referrals ?? 0 }, "Referral count fetched");
    } catch (err) {
