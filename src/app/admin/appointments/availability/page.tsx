...
const addDateAvailability = () => {
  ...
  setUsedDates([...usedDates, selectedDate]);
...
const removeDay = async (date: Date) => {
  try {
    ...
    if (data.success) {
      ...
      toast.error(data.message || "Failed to remove availability");
    }
  } catch (error) {
  ...
  setAvailability(
    availability.map((item) =>
      item.date === date
        ? {
          ...item,
          blocks: [...item.blocks, { startTime: "09:00", endTime: "10:00" }],
        }
        : item
    )
  );
...
const disableDates = (date: Date) => {
  return usedDates.some(
    (usedDate) => format(usedDate, "yyyy-MM-dd") === format(date, "yyyy-MM-dd")
  );
};
...
return (
  <div className="container mx-auto px-4 py-8">
    ...
    >
      ...
      <p className="text-muted-foreground">
        Set up your available days and time slots for appointments
      </p>
    </motion.div>
    ...
    <Card className="mb-8">
      <CardHeader>
        ...
        <CardDescription>
          Set the default duration for each appointment slot
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex items-center gap-4">
          ...
          <Button onClick={saveDurationSettings} disabled={isLoading}>
            <Save className="w-4 h-4 mr-2" />
            Save Duration
          </Button>
        </div>
    ...
    <Card className="mb-8">
      <CardHeader>
        ...
        <CardDescription>
          Select a date to configure available time blocks
        </CardDescription>
      </CardHeader>
      <CardContent>
    ...
    {availability.length > 0 && (
      <div className="space-y-6">
          ...
          <Card key={dayIndex}>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2">
                  ...
                  {format(dayAvail.date, "EEEE, MMMM d, yyyy")}
                </CardTitle>
                <Button
                  variant="destructive"
                  size="sm"
                  onClick={() => removeDay(dayAvail.date)}
                >
            ...
            <CardContent className="space-y-4">
              ...
              {dayAvail.blocks.map((block, blockIndex) => (
                ...
                >
                  <span className="font-medium text-sm">Block {blockIndex + 1}</span>
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <Input
                      ...
                      value={block.startTime}
                      onChange={(e) =>
                        updateTimeBlock(dayAvail.date, blockIndex, "startTime", e.target.value)
                      }
                      className="w-32"
                    />
                    ...
                    <Input
                      ...
                      value={block.endTime}
                      onChange={(e) =>
                        updateTimeBlock(dayAvail.date, blockIndex, "endTime", e.target.value)
                      }
                      className="w-32"
                    />
                  </div>
              ...
              {dayAvail.blocks.length > 0 && (
                <div className="mt-4 p-4 bg-primary/5 rounded-lg">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-semibold text-sm">
                      Generated Slots ({durationMinutes} min duration):
                    </span>
                  </div>
        ...
        <div className="flex justify-end">
          <Button onClick={saveAvailability} disabled={isLoading} size="lg">
            <Save className="w-5 h-5 mr-2" />
            {isLoading ? "Saving..." : "Save All Availability"}
          </Button>
        </div>
      </div>
    )}
